...
export async function getForecastHistory(
  ...
  if (category) {
    ...
    const categoryMap = new Map(
      skuCategories.map((sc) => [sc.sku, sc.category])
    );
...
export async function getForecastMetrics(userId: string, sku?: string) {
  const where: any = { userId };
  if (sku) where.sku = sku;
  ...
  for (const forecast of forecasts) {
    const sale = sales.find(
      (s) => s.date.toISOString() === forecast.forecastDate.toISOString()
    );
    if (sale) {
      const error =
        Math.abs(forecast.baseValue - sale.quantity) / sale.quantity;
      totalError += error;
      matchedCount++;
    }
  }
